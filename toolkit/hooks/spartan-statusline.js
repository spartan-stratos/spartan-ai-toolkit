...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Spartan update available?
    let updateNotice = '';
    const cacheFile = path.join(claudeDir, 'cache', 'spartan-update-check.json');
    if (fs.existsSync(cacheFile)) {
      try {
        const cache = JSON.parse(fs.readFileSync(cacheFile, 'utf8'));
        if (cache.update_available) {
          updateNotice = '\x1b[33m\u2B06 /spartan:update\x1b[0m \u2502 ';
        }
        if (cache.stale_hooks && cache.stale_hooks.length > 0) {
          updateNotice += '\x1b[31m\u26A0 stale hooks \u2014 run /spartan:update\x1b[0m \u2502 ';
        }
      } catch (e) {}
    }
